...
The syncer runs the actions described in sd TaskerCLI_Sync and returns the teamMember to TeamMemberDB, which passes it on to the
...
to sync the databases again, using sd TaskerCLI_Sync. Because of the priority, the changes made locally will override TaskerSRV if it is available.
