...
Unlike the deployment diagram above, the component diagram below depicts the interaction of different modules or components of the proposed system. There are four major components that form the entire system. Only a black view box is shown here. This means that the components are not further decomposed to reveal the inner components. The opposite of this is a white box view which shows not only the major component but also the inner components that form the big picture.  
...
TaskerSRV - This component, representing the database, consists of 1 provided interface (solid circle) and 2 required interfaces (cup shaped). These are:
\begin{itemize}
	...
	\item HTTP (required interface): Without the HTTP protocol the database would not be accessed by the taskMAN and no operation can take place.
	\item JDBC (required interface): the interface is required so that communication with the sql can be established and give the required services to the client.
\end{itemize}
TaskerMAN - This component, representing the web application has 1 required interface (SQL) and 1 provided interface (HTTP). Without SQL provided by TaskerSRV, TaskerMAN will not be able to do anything.  It will remain a useless dummy.  Also if it can't provide the HTTP communication interface then it wouldn't be able to access the database. \\
\newline 
TaskerCLI - This component representing the client has 1 provided interface (JDBC).  Since its using java to connect to the database, taskerSRV as well as SQLite will require that the interface JDBC be implemented on the client or else there will be no database service. \\
\newline SQLite - This component representing the local database on the client has 1 required interface (JDBC) which makes it possible for the client to connect to it.
\clearpage
